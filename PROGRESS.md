...
**Where to continue (2026-09-25):** Sessions 1 to 6 are done. Session 7 is built and waiting on one Mac run, then review; `REPLAN-STATE.md` "Where to continue" lists what is built and what comes next. How a session is rebuilt: `AGENTS.md` 2f.
...
| 6 | 4 | Learning Over the Graph: Embeddings and Graph Neural Networks | **Done** (approved by the instructor 2026-09-25): 42 slides, 185 minutes, generator `scripts/deckgen-s6/` | **Done** (approved 2026-09-25): on Session 5's Brunel graph, verified on macOS with Python 3.12; the old synthetic lab files are still there, to be deleted only with the instructor's agreement |
| 7 | 5 | The Agentic Query Layer, Deployment, and Open Problems | Draft (2026-09-25): `lectures/kr-session-07-new.html`, 47 slides, 179 minutes, `scripts/deckgen-s7/`; audit and visual pass clean; the model's scores wait for the Mac recording | Built (`demos/session-07-access-layer/`, OVERVIEW and README to 2e); live Gemini with a replay fallback; waiting on one `record_llm.py` run on the Mac |
...
`lu-flow` diagrams. `index.html` has cards for Sessions 1 to 6; Session 7's card is added
...
Historical (before the 2026-09-24 rebuild). The current lab is described in
`demos/session-03-ontology/README.md`; this section is kept as a record.
...
See `REPLAN-STATE.md` "Where to continue". Sessions 1 to 6 are rebuilt and approved; Session 7
waits on one Mac run and then review. Then: the Ontop run for Session 5 on the Mac, then Sessions 7 and 8

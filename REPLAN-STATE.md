...
1. **Session 4 review (item 9).** Draft deck `lectures/kr-session-04-new.html`
   and the rebuilt lab wait for the instructor. Open on his Mac: run
   `python fetch_epcis.py` once (GS1 is not reachable from Cowork) and
   capture `assets/img/s4-shacl-play-report.png` for the slide "The tool:
   shacl-play". After approval: replace `lectures/kr-session-04.html`, update
   its index card, mark Session 4 done in `PROGRESS.md`.
2. **Then Session 5, then 6 (item 9).** Session 5's code hardcodes
   `demos/session-04-shacl/shapes_template.ttl`, `ul:PurchaseOrder` and the
   invented carriers (dhl, aramex); its rebuild moves it to the Session 4
   shapes and the Brunel cast, and fixes the 1,209 untyped rate band
   carriers in the mapping (a Session 4 triage decision).
...
of truth: never commit from a copy. Not yet run for real: the Ontop path of Session 5.

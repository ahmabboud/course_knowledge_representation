...
lectures/
  ...
  kr-session-03.html           The worked example (generated by scripts/deckgen-s3). Read it before writing your own.
```
...
| **9. Vary the visuals.** | Added 2026-09-25, after the instructor approved Session 3 and named its variety as what worked. Pick the picture type that fits each idea, and mix them across a deck: step-by-step flow walks (`lu-flow`) for chains of reasoning, set pictures (inline SVG) for classes and restrictions, cards and tables for comparisons, real code and real screenshots for tools, and interactive checks (MCQ, sort, blanks, poll). Never default to one type for every slide. |
